...
// 🔑 ⚠️ APNI MOBILE APP KI KEYS YAHAN PASTE KAREIN
const SMS_API_URL = "YOUR_APP_IP_OR_URL_HERE"; // e.g., http://192.168.1.10:8080/send
const SMS_API_KEY = "YOUR_APP_API_KEY_HERE";   // Jo key app se mili hai
...
export default function Home() {
  ...
  useEffect(() => {
    setRatesUpdateTime("11 Jun 2026 at 06:50 PM");
  ...
  // 📲 REAL HARDWARE SMS GATEWAY ENGINE (CONNECTED TO YOUR DEVICE)
  const handlePhoneAuthSubmit = async () => {
    ...
    (window as any).currentSystemOtp = generatedOtp; // Saves code to memory barrier

    try {
      // 2. Fire Request directly to your Android device network terminal
      ...
      alert(lang === 'ur' ? "او ٹی پی آپ کے نمبر پر بھیج دیا گیا ہے!" : "Real OTP token fired via your mobile subscription plan!");
    } catch (err) {
      // Fallback active for seamless debugging if app server sleeps
      setShowOtpScreen(true);
      alert("Demo Mode Triggered: Input code 7861 (Device API endpoint not reached yet).");
  ...
  const handlePostAdLiveSubmit = () => {
    ...
    setVisibleAds((prev) => [newAdRow, ...prev]); // Live feed dynamic injection
    ...
    <div className="min-h-screen bg-[#f2f6fa] text-left" style={{ fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif' }}>
      ...
      {currentPage !== 'home' && (
        <main className="max-w-xl mx-auto p-4 mt-2">
          ...
          {/* PAGE 1: FIXED OTP SYSTEM & GOOGLE HYBRID LOGIN GATE */}
          {currentPage === 'page1' && (
