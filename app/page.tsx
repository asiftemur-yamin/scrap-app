...
// 1. EXTENDED ADS DATABASE (With Category Alignment)
...
const translations: any = {
  en: {
    ...
    searchPlaceholder: "Search scrap iron, plastic, copper...", browseTitle: "Browse Scrap Categories (Click to Filter Feed)",
    ...
    b2bTitle: "Verified Factories & Commercial Yards 👑 (Click to View rates)", verifyActionBtn: "Register Factory Store ⭐",
...
export default function Home() {
  ...
  // Navigation Section Refs for Scrolling Actions
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setLmeRates(prev => prev.map(m => {
        ...
        return { ...m, price: Math.max(1000, m.price + delta), up: delta >= 0, change: `${delta >= 0 ? '+' : ''}${(Math.random() * 1.5).toFixed(1)}%` };
      }));
    }, 4000);
  ...
  const getFilteredAndSortedAds = (): any[] => {
    let dataset = initialAdsData;

    if (searchQuery.trim() !== '') {
      const query = searchQuery.toLowerCase();
      dataset = dataset.filter(ad => ad.titleEn.toLowerCase().includes(query) || ad.titleUr.includes(query) || ad.desc.toLowerCase().includes(query));
    }

    if (activeMainCatFilter !== 'all') {
      dataset = dataset.filter(ad => ad.mainCat === activeMainCatFilter);
    }

    if (activeSubCatFilter !== 'all') {
      dataset = dataset.filter(ad => ad.subCat === activeSubCatFilter);
    }

    if (activeOriginFilter !== 'all') {
      dataset = dataset.filter(ad => ad.origin === activeOriginFilter);
    }

    const localAds = dataset.filter(ad => ad.city === selectedCity);
    const fallbackAds = dataset.filter(ad => ad.city !== selectedCity);

    return [
      ...localAds.map(a => ({ ...a, radiusLevel: 'local' })),
      ...fallbackAds.map(a => ({ ...a, radiusLevel: 'regional' }))
    ];
  };

  const sortedFeedAds = getFilteredAndSortedAds().slice(0, visibleAdsCount);
  ...
  return (
    <div className="min-h-screen bg-[#f2f6fa] pb-24 relative" style={{ fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif', textAlign: lang === 'ur' ? 'right' : 'left' }} dir={lang === 'ur' ? 'rtl' : 'ltr'}>
      ...
      {showSplash && (
        <div className="fixed inset-0 bg-[#1a365d] z-[999] flex flex-col items-center justify-center text-white p-6">
          <div className="text-center space-y-4">
            ...
            <p className="text-xs font-bold text-slate-300 uppercase tracking-widest">Pakistan's B2B Scrap Directory</p>
          </div>
        </div>
      )}
      ...
      <header className="bg-[#1a365d] text-white px-4 pt-4 pb-6 shadow-md rounded-b-3xl">
        ...
        {/* Defined Quick Navigation Row Actions */}
        ...
        <div className="mt-2 bg-white rounded-lg p-3 flex items-center shadow-inner text-slate-700">
          ...
          {searchQuery && <button onClick={() => setSearchQuery('')} className="text-slate-400 font-black text-xs px-1">✕</button>}
        </div>
      </header>
      ...
      <main className="px-4 mt-6">
        ...
        {/* Defined Origin Stream Toggle Filters */}
        ...
        {/* Categories Matrix Grid System with Action Labels */}
        <div className="mb-4 flex justify-between items-center">
          <h2 className="text-base font-extrabold text-slate-800 uppercase tracking-wide">{t.browseTitle}</h2>
          {activeMainCatFilter !== 'all' && <button onClick={() => { setActiveMainCatFilter('all'); setActiveSubCatFilter('all'); }} className="text-xs text-red-600 font-bold">Clear Filter ✕</button>}
        </div>
        <div className="grid grid-cols-4 gap-2.5 mb-6">
          {[
            { cat: 'iron', label: t.cat1, icon: "🔩" }, { cat: 'plastic', label: t.cat2, icon: "🛢️" },
            { cat: 'copper', label: t.cat3, icon: "🔌" }, { cat: 'aluminum', label: t.cat4, icon: "🥫" },
            { cat: 'batteries', label: t.cat5, icon: "🔋" }, { cat: 'solar', label: t.cat6, icon: "☀️" },
            { cat: 'chaaloo', label: t.cat7, icon: "⚡", trigger: () => setShowChaalooModal(true) }, 
            { cat: 'electronic', label: t.cat8, icon: "💻" }
          ].map((item, idx) => (
            <div key={idx} onClick={() => { if(item.trigger) { item.trigger(); } else { setActiveMainCatFilter(item.cat); setActiveSubCatFilter('all'); } }} className={`border rounded-xl p-2.5 flex flex-col items-center justify-center text-center shadow-sm aspect-square cursor-pointer transition-all active:scale-95 ${activeMainCatFilter === item.cat || (item.cat === 'chaaloo' && activeMainCatFilter === 'chaaloo') ? 'bg-indigo-600 border-indigo-600 text-white' : 'bg-white border-slate-100 text-slate-700'}`}>
              <span className="text-xl mb-1">{item.icon}</span>
              <span className="text-[10px] font-black leading-tight block truncate w-full">{item.label}</span>
              <span className="text-[7px] block opacity-60 mt-0.5 font-bold uppercase">{item.cat === 'chaaloo' ? 'Open List' : 'Filter'}</span>
            </div>
          ))}
        </div>
            ...
            <div className="bg-white w-full max-w-md rounded-t-3xl sm:rounded-2xl p-5 shadow-2xl space-y-4">
              ...
              <div className="space-y-2">
                {Object.keys(t.subCats).map((key) => (
                  <button key={key} onClick={() => { setActiveMainCatFilter('chaaloo'); setActiveSubCatFilter(key); setShowChaalooModal(false); }} className={`w-full text-left p-3.5 rounded-xl border font-bold text-xs flex justify-between items-center ${activeSubCatFilter === key && activeMainCatFilter === 'chaaloo' ? 'bg-indigo-600 text-white' : 'bg-slate-50 text-slate-700'}`}><span>{t.subCats[key]}</span><span className="text-[9px] opacity-60">(Select)</span></button>
                ))}
              </div>
            </div>
          </div>
        ...
        <div ref={ratesRef} className="pt-4 scroll-mt-4">
          ...
          <div className="flex gap-1.5 overflow-x-auto pb-4 scrollbar-none">
            {Object.keys(scrapRates).map((cityKey) => (
              <button key={cityKey} onClick={() => setSelectedCity(cityKey as any)} className={`px-4 py-2 text-xs font-black rounded-xl border transition-all whitespace-nowrap ${selectedCity === cityKey ? 'bg-blue-900 text-white border-blue-900 shadow-sm' : 'bg-white text-slate-600 border-slate-200'}`}>📍 {(t.cities as any)[cityKey]} <span className="block text-[8px] opacity-75 font-normal">(Change Rates)</span></button>
            ))}
          </div>
        </div>
        ...
        <div ref={feedRef} className="pt-4 border-t border-slate-200 scroll-mt-4">
          <h2 className="text-lg font-black text-slate-900 tracking-tight flex items-center justify-between mb-4">
          ...
          <div className="space-y-3.5 mb-6">
            {sortedFeedAds.length > 0 ? (
              sortedFeedAds.map((ad) => (
                <div key={ad.id} onClick={() => setSelectedAd(ad)} className={`bg-white rounded-2xl p-4 border flex justify-between items-center shadow-sm hover:shadow-md cursor-pointer relative overflow-hidden transition-all ${ad.isFeatured ? 'border-amber-300 ring-2 ring-amber-500/5' : 'border-slate-200/80'}`}>
                  <span className={`absolute top-0 right-0 text-[9px] font-black uppercase px-2.5 py-0.5 rounded-bl-xl text-white ${ad.radiusLevel === 'local' ? 'bg-blue-600' : 'bg-orange-500'}`}>{ad.radiusLevel === 'local' ? t.localBadge : t.regionalBadge}</span>
                  <div className="flex items-start space-x-3.5 gap-3 max-w-[70%]">
                    <div className="w-16 h-16 bg-slate-100 rounded-xl border flex flex-col items-center justify-center text-3xl shrink-0">{ad.icon}</div>
                    <div className="space-y-0.5">
                      {ad.isFeatured && <span className="text-[9px] bg-amber-100 text-amber-800 px-1.5 py-0.5 rounded font-black inline-block mb-1">⭐ FEATURED</span>}
                      ...
                      <span className="text-[8px] text-blue-600 font-bold block uppercase mt-1">Click to view details & contact</span>
                    </div>
                  </div>
                  <div className="text-right shrink-0"><span className="text-base font-black text-green-600 block">Rs.{ad.price}</span><span className="text-[9px] font-black text-slate-400 block">/{ad.unit}</span></div>
                </div>
              ))
            ) : (
              <div className="text-center py-12 bg-white rounded-2xl border border-dashed border-slate-300"><span className="text-4xl block mb-2">📦</span><p className="text-sm font-bold text-slate-400">No Ads Found Matching Your Filters.</p></div>
            )}
          </div>
        </div>
      </main>
          ...
          <div className="bg-[#f2f6fa] w-full max-w-lg rounded-t-3xl sm:rounded-2xl max-h-[92vh] overflow-y-auto shadow-2xl relative flex flex-col pb-6">
            ...
            <div className="p-5 space-y-4 text-left" dir={lang === 'ur' ? 'rtl' : 'ltr'}>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-white border rounded-xl p-3 shadow-sm"><span className="text-xs font-bold text-slate-400 block">Total Weight</span><span className="text-sm font-black text-slate-700">⚖️ {selectedAd.weight}</span></div>
                <div className="bg-white border rounded-xl p-3 shadow-sm"><span className="text-xs font-bold text-slate-400 block">City Origin</span><span className="text-sm font-black text-slate-700 capitalize">📍 {(t.cities as any)[selectedAd.city] || selectedAd.city}</span></div>
              </div>
              <div className="bg-white border rounded-2xl p-4 shadow-sm"><p className="text-xs font-medium text-slate-600 leading-relaxed">{selectedAd.desc}</p></div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[125] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-3xl p-6 shadow-2xl relative space-y-5">
            ...
            <div className="space-y-4 text-left" dir={lang === 'ur' ? 'rtl' : 'ltr'}>
              <div><label className="block text-xs font-bold text-slate-700 mb-1">{t.companyNameLabel}</label><input type="text" placeholder="e.g., R-H-A-F Smelting Plant" className="w-full bg-slate-50 border rounded-xl p-3 text-xs outline-none focus:border-amber-500 font-bold" /></div>
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 text-center"><span className="text-[10px] text-amber-800 font-black block uppercase">Premium Security Verification Fee</span><span className="text-base font-black text-amber-600 block mt-0.5">Rs. 12,500 / Year</span></div>
              <button onClick={() => { alert("Premium Registration Activated Successfully!"); setShowVerificationPortal(false); }} className="w-full bg-gradient-to-r from-amber-500 to-amber-600 text-white font-black text-sm py-4 rounded-xl shadow-md">{t.payFeeBtn}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showInbox && (
        <div className="fixed inset-0 bg-[#f2f6fa] z-[105] flex flex-col">
          <div className="bg-[#1a365d] text-white p-4 sticky top-0 flex justify-between items-center shadow-md">
            ...
            <button onClick={() => setShowInbox(false)} className="text-xs bg-red-600 text-white px-3 py-1.5 rounded-xl font-bold">✕</button>
          </div>
          <div className="flex-1 flex flex-col justify-between bg-slate-50 overflow-hidden">
            <div className="p-3 bg-white border-t flex gap-2 items-center">
              <input type="text" placeholder={t.chatPlaceholder} className="w-full bg-slate-100 border rounded-xl p-3 outline-none text-xs font-bold text-slate-800" />
              <button className="bg-indigo-600 text-white font-black px-5 py-3 rounded-xl text-xs">Send</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showAuth && (
        <div className="fixed inset-0 bg-[#f2f6fa] z-[300] flex flex-col justify-center p-4">
          <div className="max-w-md w-full mx-auto bg-white rounded-3xl shadow-2xl border p-6 space-y-5 relative">
            ...
            <div className="space-y-4 text-left" dir={lang === 'ur' ? 'rtl' : 'ltr'}>
              <div><label className="block text-xs font-black text-slate-700 mb-1">{t.emailLabel}</label><input type="email" placeholder="name@email.com" className="w-full bg-slate-50 border rounded-xl p-3 text-xs outline-none focus:border-[#0066cc] font-bold" /></div>
              <div><label className="block text-xs font-black text-slate-700 mb-1">{t.passLabel}</label><input type="password" placeholder="••••••••" className="w-full bg-slate-50 border rounded-xl p-3 text-xs outline-none focus:border-[#0066cc] font-bold" /></div>
              <button onClick={() => { setIsLoggedIn(true); setShowAuth(false); setShowPostAd(true); }} className="w-full bg-[#0066cc] text-white font-black text-sm py-3.5 rounded-xl shadow-md mt-2">Sign In Successfully 🔑</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showPostAd && isLoggedIn && (
        <div className="fixed inset-0 bg-[#f2f6fa] z-[400] flex flex-col overflow-y-auto pb-12">
          ...
          <div className="p-5 max-w-lg mx-auto w-full space-y-5" dir={lang === 'ur' ? 'rtl' : 'ltr'}>
            <div><label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Product Title / Maal Ka Naam</label><input type="text" placeholder="e.g., Structural Steel HMS 1 Lot" className="w-full bg-white border rounded-xl p-3.5 text-sm outline-none focus:border-[#0066cc] font-bold" /></div>
            <div>
              <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Main Category</label>
              <select value={formMainCat} onChange={(e) => { setFormMainCat(e.target.value); if(e.target.value !== 'chaaloo') setFormSubCat('scrap'); }} className="w-full bg-white border rounded-xl p-3.5 text-sm outline-none focus:border-[#0066cc] font-bold text-slate-700">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div><label className="block text-xs font-bold text-slate-700 mb-1">Weight</label><input type="text" placeholder="e.g., 500 Kg" className="w-full bg-white border rounded-xl p-3.5 text-sm outline-none font-bold" /></div>
            ...
            <button type="button" onClick={() => { alert("Ad Published Successfully on Main Grid!"); setShowPostAd(false); }} className="w-full bg-green-600 text-white font-black text-sm py-4 rounded-xl shadow-lg mt-6">Publish Advertisement 📢</button>
          </div>
        </div>
      )}

      {/* Sticky Bottom Navigation Bar Controls with Action Descriptions */}
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 px-2 py-2 flex justify-around items-center z-50 shadow-lg">
        <button onClick={() => { setShowInbox(false); setShowPostAd(false); setActiveMainCatFilter('all'); setActiveSubCatFilter('all'); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className="flex flex-col items-center text-[#0066cc] font-bold text-[10px] w-14">
          <span className="text-lg">🏠</span><span className="mt-0.5">{t.navHome}</span><span className="text-[7px] text-slate-400 font-normal">(Refresh)</span>
        </button>
        <button onClick={() => scrollToSection(feedRef)} className="flex flex-col items-center text-slate-400 font-medium text-[10px] w-14">
          <span className="text-lg">📋</span><span className="mt-0.5">{t.navAds}</span><span className="text-[7px] text-slate-400 font-normal">(Market)</span>
        </button>
        ...
        <button onClick={() => setShowInbox(true)} className="flex flex-col items-center text-slate-400 font-medium text-[10px] w-14">
          <span className="text-lg">💬</span><span className="mt-0.5">{t.navChat}</span><span className="text-[7px] text-slate-400 font-normal">(Inbox)</span>
        </button>
        <button onClick={() => scrollToSection(ratesRef)} className="flex flex-col items-center text-slate-400 font-medium text-[10px] w-14">
          <span className="text-lg">💰</span><span className="mt-0.5">Rates</span><span className="text-[7px] text-slate-400 font-normal">(Mandi)</span>
        </button>
      </nav>
