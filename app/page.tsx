...
const SUPABASE_URL = "https://fxybqucvtewkylctrljoj.supabase.co"; //
const SUPABASE_KEY = "sb_publishable_drme4BfnnvyMX1gkyfCyrA_s9chTPsg"; //

// 🔑 AAP KI REAL WABLAS WHATSAPP GATEWAY CONFIGURATION (ALREADY ADDED)
const WHATSAPP_API_URL = "https://deu.wablas.com/api/v2/send-message"; 
const WHATSAPP_TOKEN = "xarPBvDy9FABYa2dFnMYksyq3WWExq8YU01"; // Aap ki real key fix kar di hai
...
export default function Home() {
  ...
  // 📲 REAL WHATSAPP OTP ENGINE (CONNECTED TO WABLAS)
  const handlePhoneAuthSubmit = async () => { //
    ...
    try { //
      // 2. Fire Request to Wablas WhatsApp API Gateway
      ...
      alert(lang === 'ur' ? "او ٹی پی کوڈ آپ کے واٹس ایپ پر بھیج دیا گیا ہے!" : "Real OTP token fired to your WhatsApp via Wablas!");
    } catch (err) { //
      console.error(err);
      alert("Wablas Connection Error! Check your Nano package status."); //
    } //
  }; //

  const handleVerifyOtpCode = () => { //
    const realSystemOtp = (window as any).currentSystemOtp; //
    if (inputOtp === realSystemOtp) { //
      setIsLoggedIn(true); //
    ...
    <div className="min-h-screen bg-[#f2f6fa] text-left" style={{ fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif' }}>
      ...
      {currentPage !== 'home' && ( //
        <main className="max-w-xl mx-auto p-4 mt-2">
          ...
          {/* PAGE 1: FIXED WHATSAPP OTP SYSTEM & GOOGLE HYBRID LOGIN GATE */}
          {currentPage === 'page1' && ( //
              ...
              {!showOtpScreen ? ( //
                <div className="space-y-4">
                  <div className="space-y-1">
                    ...
                    <input type="tel" value={inputPhone} onChange={(e) => setInputPhone(e.target.value)} placeholder="62812345678" className="w-full bg-slate-50 border rounded-xl p-3.5 text-sm font-black outline-none" />
                    <span className="text-[10px] text-slate-400 block font-bold mt-1">* Note: Type number with Country Code (e.g., 923001234567)</span>
                  </div>
                  <button onClick={handlePhoneAuthSubmit} className="w-full bg-gradient-to-r from-[#1a365d] to-[#0f2444] text-white font-black py-4 rounded-xl text-xs uppercase shadow-sm">Send WhatsApp OTP 📲</button>
